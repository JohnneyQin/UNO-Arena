...
from Host.agentPlayer.prompt.prompt_SA_Arena import (select_card_prompt,
                                                     select_color_prompt,
...
class agentPlayer:
    ...
    def select_card(self, game=None):
        ...
        for index in opponents:
            len_cards.append(len(players[index].hand))
    ...
    def select_challenge_black(self, game=None, new_color=None, ID=None):
        player_id = self.player_id
        opponent_id = (player_id + 1) % 2
        len_deck = len(game.deck)
        len_discard_pile = len(game.discard_pile)
